...
import com.asio.tools.enumutils.enums.CommonEnum;
...
public class EnumUtils {
     ...
     */
    public static <E, K, V> Map<K, V> enumToMap(Class<E> enumClass, String keyFieldName, String valueFieldName, Map<K, V> targetMap) {
        E[] enumConstants = enumClass.getEnumConstants();
        for (E elementEnum : enumConstants) {
            K keyValue;
            V value;
    ...
    public static void main(String[] args) {
        // 获取指定枚举类指定类型值的枚举
        /*TestEnum testEnum = EnumUtils.valueOf(TestEnum.class, EnumUtil.FIELD_CODE, "java");*/

        //将枚BaseEnum子类举中的字段转为key-value对象【EnumProperty】集合
        /* List<EnumProperty<Integer, String>> enumProperties = EnumUtils.toBaseEnumKeyAndValueList(TestEnum.class, EnumUtil.FIELD_VALUE, EnumUtil.FIELD_NAME);
        System.out.println(JSONUtil.toJsonStr(enumProperties));*/

        //将常规枚举中的字段转为key-value对象【EnumProperty】集合
        /*List<EnumProperty<String, Double>> commonEnumKeyAndValueList = EnumUtils.toCommonEnumKeyAndValueList(CommonEnum.class, "code", "price");
        ...
        //将常规枚举中的指定字段转为list集合
       /* List<String> commonNameList = EnumUtils.listEnumField(CommonEnum.class, CommonEnum.FIELD_NAME);
        List<String> commonPriceList = EnumUtils.listEnumField(CommonEnum.class, CommonEnum.FIELD_PRICE);
        System.out.println(commonNameList + " -- " + commonPriceList);*/

        //将常规枚举按指定字段为key，枚举对象为value转为map集合
        /*Map<String, CommonEnum> commonEnumMap = EnumUtils.enumToMap(CommonEnum.class, "name");
        System.out.println(commonEnumMap);
        System.out.println("苹果价格: " + commonEnumMap.get("苹果").getPrice());*/

        //将常规枚举按指定字段为key，指定字段为value转为map集合
        Map<String, Double> nameAndPriceMap = EnumUtils.enumToMap(CommonEnum.class, "name", "price", new HashMap<String, Double>());
